/**
 * Main Application (Latest Version with Fuzzy Matching)
 * Orchestrates the dashboard functionality
 * This version uses StateConfigLatest with fuzzy matching for schedule state normalization
 */
...
const App = {
    ...
    _populateFilterOptions() {
        ...
        combined.forEach(entry => {
            ...
            const normalizedState = StateConfigLatest.findMatchingState(entry.scheduleState);
            if (normalizedState) {
                stateSet.add(normalizedState);
            }
        });

        const categories = StateConfigLatest.getAllCategories() || [];
        const groups = StateConfigLatest.getAllGroups() || [];
    ...
    _renderDashboardToggleBar() {
        ...
        const visibility = StateConfigLatest.getDashboardVisibility();
    ...
    _toggleDashboardVisibility(groupName) {
        ...
        StateConfigLatest.setDashboardVisibility(groupName, !currentlyVisible);
        this._renderDashboardToggleBar();
    ...
    _setAllDashboardsVisibility(visible) {
        StateConfigLatest.setAllDashboardVisibility(visible);
        this._renderDashboardToggleBar();
        if (this.processedPrevious && this.processedUpdated) {
            DashboardRenderer.render(this.processedPrevious, this.processedUpdated);
        } else {
    ...
    _applyFilters(dataset) {
        ...
        return dataset.filter(entry => {
            const normalizedState = StateConfigLatest.findMatchingState(entry.scheduleState);
            const stateConfig = StateConfigLatest.getStateByName(normalizedState);
